  ...
  switch (action.type) {
    ...
    case "SET_STATUS":
      return {
        ...
        children: action.status === "single" ? [] : state.children,
      };

    case "ADD_CHILD":
      return {
        ...state,
        children: [...state.children, { firstName: "", lastName: "" }],
      };
...
export default function MemberModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      status: state.status,
      children: state.status === "family" ? state.children : [],
    };
      ...
      <form onSubmit={handleSubmit} className="space-y-6 p-5">
        ...
        <div className="flex gap-4">
          <button
            type="button"
            onClick={() =>
              dispatch({ type: "SET_STATUS", status: "single" })
            }
            className={`flex-1 border p-2 rounded ${
              state.status === "single" ? "bg-blue-100" : ""
            }`}
          >
            Single
          </button>
          <button
            type="button"
            onClick={() =>
              dispatch({ type: "SET_STATUS", status: "family" })
            }
            className={`flex-1 border p-2 rounded ${
              state.status === "family" ? "bg-blue-100" : ""
            }`}
          >
            Family
          </button>
            ...
            {state.children.map((child, i) => (
              <div key={i} className="flex gap-2">
                ...
                <button
                  type="button"
                  onClick={() =>
                    dispatch({ type: "REMOVE_CHILD", index: i })
                  }
