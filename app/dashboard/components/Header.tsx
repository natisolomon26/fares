...
import { Bell, Search, Calendar, Menu, User, ChevronDown } from 'lucide-react'
    ...
    <header className="sticky top-0 z-40 bg-emerald-900/90 backdrop-blur-xl border-b border-emerald-800/50 shadow-lg shadow-emerald-900/20">
      <div className="px-4 lg:px-6 py-4">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="hidden xl:block text-left">
                    <p className="font-semibold text-yellow-100">Pastor John</p>
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-64 bg-emerald-900/95 backdrop-blur-xl rounded-xl border border-emerald-800/50 shadow-2xl shadow-black/30 py-2 z-20">
                    <div className="px-4 py-3 border-b border-emerald-800/30">
                      <p className="font-semibold text-yellow-100">Pastor John</p>
                      <p className="text-sm text-emerald-100/60">pastor@gracechurch.org</p>
                    </div>
                    <div className="py-2">
                      {[
                        { label: 'My Profile', icon: User },
                        { label: 'Account Settings', icon: Settings },
                        { label: 'Church Settings', icon: Church },
                        { label: 'Billing & Plans', icon: DollarSign }
                      ].map((item) => (
                        <button
                          key={item.label}
                          className="flex items-center gap-3 w-full px-4 py-2.5 text-emerald-100/80 hover:text-yellow-100 hover:bg-yellow-500/10 transition-all"
                        >
                    ...
                    <div className="border-t border-emerald-800/30 pt-2">
                      <button className="flex items-center gap-3 w-full px-4 py-2.5 text-emerald-100/80 hover:text-red-300 hover:bg-red-500/10 transition-all">
        ...
        <div className="hidden lg:flex items-center gap-6 mt-4 pt-4 border-t border-emerald-800/30">
          {[
            { label: 'Today\'s Service', value: '9:00 AM', color: 'text-yellow-300' },
            { label: 'Attendance', value: '856', color: 'text-emerald-300' },
            { label: 'Prayer Requests', value: '12', color: 'text-blue-300' },
            { label: 'New Members', value: '5', color: 'text-purple-300' },
          ].map((stat, idx) => (
...
import { Settings, Church, DollarSign, LogOut } from 'lucide-react'
